...
import ru.chessfactory.pgn.analysis.core.GameTypes;
...
public class GameToAggregatesTransformer extends AbstractTransformer {
    ...
    private GameAggregates handleGame(Message<Game> item) {
        ...
        GameAggregates agg = playback.collectAggregates();
        populateWithHeaderFields(item, agg);
    ...
    private void populateWithHeaderFields(Message<Game> item, GameAggregates agg) {
        Game game = item.getPayload();
